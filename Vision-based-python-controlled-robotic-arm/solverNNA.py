...
def get_previous_teta2():
    try:
        ...
        # Split on semicolon and remove any empty entries
        prev_teta = [int(val) for val in data.split(';') if val.strip() != '']

        if len(prev_teta) > 6:
            prev_teta.pop(6)  # If needed

        return prev_teta

    except FileNotFoundError:
        print("[ERROR] prev_teta.txt not found.")
        return [0, 0, 0, 0, 0, 0]

    except Exception as e:
        print(f"[ERROR] Unexpected issue in get_previous_teta2(): {e}")
        return [0, 0, 0, 0, 0, 0]
